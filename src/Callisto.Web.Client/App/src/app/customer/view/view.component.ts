...
import { CustomerViewModel } from '../models/CustomerViewModels';
...
export class ViewComponent implements OnInit, OnDestroy {
  ...
  loadMembers() {
    this.customerService.GetCustomerMembers().subscribe(c => {
  ...
  removeMember(id: string) {
    this.alertService.showDialog('Do you want to remove this item?', 'Are you sure?', MessageSeverity.warn, x => {
      this.customerService.RemoveCustomerMember(id).subscribe(c => {
        if (c.Status !== RequestStatus.Success) {
          this.alertService.showWarningMessage(c.FriendlyMessage);
        } else {
          this.loadMembers();
